...
カフェは背景・家具・購入設備・テーブルと座席・客・恋愛対象キャラクター・吹き出し・演出の8レイヤーです。注文の吹き出しから調理・提供でき、詳細は「注文とキッチン」、天気・設備・スタッフ・思い出・累計は「店のようす」から開きます。既存のゲーム処理・データ・v5セーブ形式は変更していません。

素材の配置先・ファイル名・推奨サイズは [カフェ素材ガイド](public/assets/cafe/README.md) を参照してください。PNGを追加すると項目ごとにCSSの仮素材から置き換わります。ゲームのタイマーに追従する来店と退店の演出、通常客4種、スタッフの作業位置、設備の湯気、天気に応じた光と雨を表示します。人物をタップすると既存の人物詳細を開きます。

店長（プレイヤー）は常に店内に表示されます。注文の「調理開始」を選ぶと対象設備へ歩いて調理を始め、完成後の「提供する」で料理を取り、中央の通路から客席へ運びます。複数の指示は順に実行します。既存スタッフは従来どおり並行して自動調理・提供します。店長の見た目は `public/assets/characters/manager.png` から差し替えられます。
