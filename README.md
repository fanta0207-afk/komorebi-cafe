...
## 開店ミッション・経済調整（保存形式 v7）

- 37個の開店ミッションを追加。最初の注文／調理／完成／提供から、蓮との出会い、1パックずつの発注と入荷確認、会話と短編集の贈物、コーヒー3・5・8・10杯、共同開発「ふたりのカフェモカ」、材料調達と初提供、550コインの貯蓄とカウンター強化まで案内します。
- 画面上部に現在のミッションと行き先ボタンを常設。「手順・進捗を見る」から37件すべての条件・報酬・達成状態を確認できます。物語が始まる際はノート類を閉じ、物語が隠れないようにします。
- 各報酬は5〜30コイン、1回限り。達成後に順番に受け取ります。先に済ませた行動は記録され、食材消費や画面移動で達成が取り消されることはありません。ミッション報酬は売上・提供件数に加算しません。
- 新規開始の所持金を3,000から200コインに変更。開始用の豆・パン各10食分と基本設備3台は維持します。設備・強化・雇用・贈物・仕入れの価格は据え置きです。
