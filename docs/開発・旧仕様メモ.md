...
README整理前の説明を保存した資料です。過去の仕様も含むため、最新の公開手順は[README](../README.md)、最新のゲーム仕様は[自動生成の仕様書](仕様書.md)を参照してください。ここにあるv5・v6・v10などの記述を現在の保存形式と取り違えないでください。

## GitHub・Vercelでの公開

### 確認した構成

ゲーム本体はReact 19 + TypeScript。既存の開発環境はVite 8上のvinextで、`app/page.tsx`・`app/layout.tsx`というNext.js互換形式とCloudflare Workerを使っています。通常のNext.jsでも、HTMLだけのゲームでもありません。

Vercel向けには同じゲームコンポーネントを`src/main.tsx`から読み込み、`vite.public.config.ts`で静的SPAを作成します。Cloudflare、データベース、ChatGPT認証は公開版に不要です。既存のvinext環境は残してあります。

### 起動・本番ビルド

Node.js 22.13以上（公開環境は22.x）、npmを使用します。以下はすべてこのREADMEのあるフォルダで実行してください。
...
表示されたlocalhostのURLを開いてください。既存の開発メニュー付き環境は`npm run dev`で起動できます。
...
本番成果物は`dist-public/`です。`dist/`は従来のCloudflare Worker用で、Vercelには指定しないでください。`public/`だけを公開してもゲームは動きません。
...
前者は公開版のビルドと素材・設定チェック、後者は既存Workerと公開版の両方のビルドおよび全ゲームテストを実行します。

公開準備時の確認結果：本番ビルド成功、全170テスト成功、320px・390px幅のブラウザ表示と直接URLの再読み込みを確認済み。`npm run lint`は19件のエラー・8件の警告が残っており、lint合格とはしていません。ビルド中の背景URL警告は素材を後からコピーする構成によるもので、該当する背景ファイルが成果物に存在することをテストで確認しています。JSのサイズ警告も残っています。Vercelへの実デプロイ後の確認は別途必要です。

### GitHubに送るフォルダ

このプロジェクトフォルダ（ローカルでは`v`）は独立したGitリポジトリです。親フォルダ側では`v`がgitlinkとして登録されているため、親だけをpushしてもゲームのソースは送られません。**このREADME・package.json・vercel.jsonが直接ルートにあるリポジトリをGitHubへ送ってください。** Gitメタデータの削除や親リポジトリの構造変更は不要です。

送信前に`git status`と`git diff`で内容を確認し、公開したくないシナリオなどがないかも確認してください。`node_modules/`、成果物、`.env*`、鍵、ログ、制作中の`outputs/`・`work/`は`.gitignore`で除外しています。`.gitignore`はすでに追跡済みのファイルや過去の履歴を消すものではありません。APIキーなどを追加した場合は環境変数に置き、`VITE_`付きの変数にも秘密情報を入れないでください。

### Vercelの入力設定

GitHubへ送った上記のリポジトリをVercelの「Add New → Project」でImportします。

| 項目 | 設定 |
| --- | --- |
| Framework Preset | Vite（Next.jsを選ばない） |
| Root Directory | `./`（このREADMEがリポジトリのルートにある場合） |
...
ビルド命令・出力先・SPA fallbackは`vercel.json`にも記載しています。将来、通常のフォルダとして`v/`を含む単一リポジトリに整理した場合だけRoot Directoryを`v`に変えてください。現在の親gitlinkのままではこの方法は使えません。

Deploy後はトップページ、`/town`・`/gifts`などの直接アクセスと再読み込み、スマホでの操作、仕入れ後の復帰を確認してください。画面切替自体はReact内の状態管理で、URLは更新しません。直接URLはゲームの入口へ戻し、画面名からその画面を復元する機能は追加していません。素材パスはfallback対象から外しており、存在しない素材をHTMLで返さない設定です。

公式資料: [VercelのVite設定・SPA公開](https://vercel.com/docs/frameworks/frontend/vite)、[Viteの静的素材](https://vite.dev/guide/assets)。

### 素材・スマートフォン・公開範囲

- 画像は同梱の`public/assets/`から`/assets/...`で参照。公開時に同じ配置でコピーします。音声ファイル・Webフォントは現時点では使っておらず、フォントは端末の日本語フォントです。
- 未制作の料理・食材・贈物・一部家具などの画像は、既存実装のCSS・絵文字による代替表示を利用します。全種類の画像が揃っているという意味ではありません。
- 日本語HTML、device-width、viewport-fit=cover、100dvh、ノッチ向けsafe-area、最大幅430pxの既存デザインを維持します。
- 公開成果物にはHTML・JS・CSS・favicon・素材だけを含めます。README、仕様書、制作資料、`.gitkeep`、サンプル、ソースマップは含めません。公開版の開発メニューも表示しません。
- GitHubが公開リポジトリならソースやシナリオは閲覧できます。Web上のJS・画像も閲覧・保存可能です。秘密にしたい情報はコードに置かないでください。

### セーブデータ（重要）

保存キー`komorebi-cafe-save-v1`と既存の読み込み・移行処理は変更していません。**調査時点の保存形式はv21で、v5ではありません。** v5データの読み込み互換性はテストで確認しますが、新たな保存をv5へ下げる変更はしていません。v5のまま書き出す必要がある場合は別途、既存v21データや新しい機能の扱いを決めてから対応してください。

セーブはブラウザのlocalStorageで、サーバーへ送信しません。同じブラウザ・同じoriginでのみ継続できます。localhost、VercelのPreview URL、本番URL、独自ドメインの間では自動移行されません。公開URLは早めに固定してください。クラウド保存・セーブ移行機能はこの公開対応には含みません。

---

## 既存ゲームの仕様・従来のvinext環境

## 仕様書

現在のゲーム仕様は [docs/仕様書.md](仕様書.md) にまとめています。キャラクター、ストーリー、食材、料理、設備、主要パラメーターは実装から自動生成されます。`npm run dev` と `npm run build` の前に自動更新されます。
