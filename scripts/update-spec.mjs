...
- 自然物の基本結果は通常食材・森食材・希少森素材・空振りのいずれか。森食材は序盤8〜10%に抑え、各層帯の設定率で出現。森食材・希少森素材は1か所につき1個。通常食材のみ${FOREST_CONFIG.abundantChance*100}%で2個発見。未発見の希少素材は50層まででは出ず、既に発見済みなら低率で再採集可能。51層以降で琥珀花蜜・月しずくベリーを狙える。
- 食事による希少率加算は最大${Math.round(FOREST_CONFIG.maxRareBonus*100)}ポイント。通常食材枠から移し、空振り率は増やさない。最奥の希少素材率は無補助25%、食事込み最大30%。
...
| 層 | エリア | 探索場所 | 道の救済回数 | 森食材率 | 基本希少率 | 空振り率 | 障害確率／最大処理 |
|---|---|---|---:|---:|---:|---:|---|
${FOREST_BANDS.map(b=>`| ${b.from}〜${b.to} | ${forestAreas.find(a=>a.id===b.area).name} | ${b.spots.join('〜')} | ${b.pathLimit} | ${Math.round(b.forestRate*100)}% | ${Math.round(b.rareRate*100)}% | ${Math.round(b.emptyRate*100)}% | ${Math.round(b.obstacleChance*100)}%／${b.obstacleMax} |`).join('\n')}
