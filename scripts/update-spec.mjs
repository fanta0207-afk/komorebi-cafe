...
- 初回導入：祖母からこもれび喫茶を託される4場面のプロローグ後、店内のミッションを強調し、最初の「街へ行く」へ案内する。初回だけ表示し、物語や案内は閉じられる。プロローグを含む複数ページの物語は「戻る」で一つ前の場面を読み直せる。
...
1. 街の仕入れ先を訪ね、食材を1〜${GAME_CONFIG.maxProcurementPacks}パックで発注する。発注通知には食材名と食分だけを表示し、到着までの時間は仕入れ画面で確認する。
...
- 料理一覧にはメニュー解放率、利益、食材の在庫、各料理の累計提供数、現在Lv.、次のLv.までの進捗、熟練売上ボーナスを表示する。食材代は0のため表示しない。隠し料理も最初から解放率の分母に含め、発見時には分子だけを増やす。
- 一時的な通知は発注・入荷・売上・解放・更新・雇用などの結果を短く伝える。物語の詳しい報酬文はニュースに残す。
...
- 設備・客席の購入カードは、今買えるものを金色の縁と緑の購入ボタンで強調する。コイン不足は不足額と価格を表示して落ち着いた色にし、物語・ミッション待ちと設置上限は別の表示で区別する。各設備の強化ボタンも購入可否と不足額を示す。購入可否は現在の所持コイン、解放状態、設置台数、調理中の状態に合わせて即時更新する。

## 11. 好感度と店づくり

- 雑貨屋は${GAME_CONFIG.giftShopSize}種類を並べ、各ギフトはその品揃えにつき1個だけ購入できる。価格・レアリティの抽選重みは従来どおり。
- 手動の品揃え更新は無料で1日${GAME_CONFIG.giftShopDailyRefreshLimit}回まで。日本時間の毎日0時に回数を回復し、未使用回数は翌日へ持ち越さない。ゲーム内の日送りでは回復しない。
- 自動更新は${GAME_CONFIG.giftShopAutoRefreshMs/3600000}時間ごと、日本時間の${Array.from({length:86400000/GAME_CONFIG.giftShopAutoRefreshMs},(_,i)=>i*GAME_CONFIG.giftShopAutoRefreshMs/3600000).join("・")}時。手動更新ではこの時刻を変更せず、回数も消費しない。閉じていた間の更新は最新の品揃えへ一度だけ反映し、過去の品揃えや購入枠は蓄積しない。
...
- v25の初回導入状態を保存する。旧セーブにはプロローグを後から表示せず、新規セーブと初期化後にだけ表示する。
- 新しいキャラクターは旧セーブの読込時に初期進行状態で自動追加する。
- 進行中の調理と仕入れの配送時間は、再読込や画面を閉じている間も実時間で進む。閉じている間に新しい来店・自動提供・売上は発生せず、再開後に完成した料理を提供する。
...
- Vercel公開版は同じReactゲームを静的SPAとして構築し、開発メニューを非表示にする。店画面では在庫ボタンの下に設定ボタンを並べ、森画面ではゲーム枠内の右上に置く。設定からセーブデータの初期化だけは利用でき、実行前に取り消し可能な確認を表示する。開発サーバーでは同じ位置から従来どおりDEVメニューを使える。ゲーム数値・保存キー・保存移行は変更しない。
...
mkdirSync(dirname(outputPath), { recursive: true });
const { FOREST_CONFIG, FOREST_COIN_BANDS, FOREST_BANDS, FOREST_MEAL_LEVELS, forestAreas, forestIngredients, forestRecipes, handmadeGifts } = missionRequire(join(missionRuntimeRoot, "data", "forest.js"));
const forestNames = Object.fromEntries([...ingredients,...forestIngredients].map(i=>[i.id,i.name]));
const forestSection = `
## 16. こもれびの森（実装済み）

- 初めて料理を1皿提供すると、街から森へ出発できる。全${FOREST_CONFIG.maxFloor}層。旧7エリアを各10層へ展開し、岩・採掘・戦闘の要素は使わない。
- 体力上限${FOREST_CONFIG.maxEnergy}。調べる・障害処理・次の層への移動は1操作につき${FOREST_CONFIG.actionCost}消費。出発・帰還・自動収納・手帳とかごの確認は無料。体力0でも帰還でき、成果は失わない。
- 探索外は実時間${FOREST_CONFIG.recoveryMs/1000}秒で体力1回復。端数を保持し、上限中の時間を貯めない。探索中はアプリを閉じても回復せず、帰還時から回復を再開する。
...
- 食事の幸運効果は小道救済回数を最大${FOREST_CONFIG.maxPathReduction}減らす（下限2）。足取りは各小道の障害処理を1〜2回省略し、各層の成果も道もない自然物を最大1〜2か所見分ける。目印のために報酬を書き換えず、対象に道・断片・コイン・券がある場所や分け合い箱を含めない。
- 初回は最初の探索で森素材を保証し、3回までに小道を発見。以後は通常抽選。限定料理7品はすべて秘密のレシピで、各10層帯に1品ずつ対応する。切れ端は対応層帯で発見でき、同じレシピは1探索で最大1枚、合計3枚で解放。
- 箱以外の自然物から食材が見つかる確率は全層${FOREST_CONFIG.findChance*100}%。結果は通常食材・森食材・希少森素材のいずれか。森食材・希少森素材は1か所につき1個。通常食材のみ${FOREST_CONFIG.abundantChance*100}%で2個発見。分け合い箱を含め、70か所の採集で食材約15個を目安とする。未発見の希少素材は50層まででは出ず、既に発見済みなら低率で再採集可能。51層以降で琥珀花蜜・月しずくベリーを狙える。
- 発見成功時の内訳は、各層帯の森食材率・希少素材率を使用。食事による希少率加算は最大${Math.round(FOREST_CONFIG.maxRareBonus*100)}ポイントで、通常食材枠から移す。最奥では発見した食材のうち希少素材が無補助25%、食事込み最大30%。食事で食材の総発見率は増えない。
- 分け合い箱は未配置の間に各場所${FOREST_CONFIG.boxChance*100}%で抽選し、1層に最大${FOREST_CONFIG.maxBoxes}個。通常食材1個が中心で、解放済みの物語限定食材があれば8%で置換。箱を調べたときにコイン${FOREST_CONFIG.coinChance*100}%、調達券${FOREST_CONFIG.ticketChance*100}%を独立抽選。自然物の探索・障害処理・移動では券やコインを抽選しない。
- コイン発見時は${FOREST_COIN_BANDS.map(b=>`${b.min===b.max?b.min:b.min+'〜'+b.max}コイン：${b.weight}%`).join('、')}の重みで、各区間内は一様抽選。調達券1枚で発注1件の全パックを即受取。確認画面には食材名・食分・使用枚数・所持枚数を表示する。自然到着・消失済みの入荷には消費しない。
- かごは容量制限なし。同じ素材をまとめ、発見時に自動収納。帰還時に店の在庫・断片・コイン・券へ一括精算し、成果一覧を表示する。受取ボタンはなく、再読込や帰還連打でも二重加算しない。
- 10・20・30・40・50・60層へ到達すると目印を保存し、次回から11・21・31・41・51・61層へ無料出発可能。近道にメニュー開発数条件はない。51層以降の森自体は成果あり帰還${FOREST_CONFIG.deepReturns}回＋累計提供${FOREST_CONFIG.deepOrders}皿で解放。徒歩と近道で同じ解放条件。
...
- DEVメニューは開発サーバーだけに表示する。コイン・注文・ギフト更新・料理全解放・仕入れ・調理・好感度・初期化に加え、こもれびの体力を70まで全回復できる。公開版では設定画面の初期化だけを残す。
...
|---|---|---|---:|---:|---:|---|
${FOREST_BANDS.map(b=>`| ${b.from}〜${b.to} | ${forestAreas.find(a=>a.id===b.area).name} | ${b.spots.join('〜')} | ${b.pathLimit} | ${Math.round(b.forestRate*100)}% | ${Math.round(b.rareRate*100)}% | ${Math.round(b.obstacleChance*100)}%／${b.obstacleMax} |`).join('\n')}

### 食事熟練度

| Lv. | 足取り：道ごとの処理省略 | 足取り：各層の目印 | 幸運：道救済短縮 | 幸運：希少率加算 |
|---|---:|---:|---:|---:|
${FOREST_MEAL_LEVELS.map(t=>`| ${t.from}〜${t.to} | ${t.obstacleSkip} | ${t.emptyHints} | ${t.pathReduction} | ${Math.round(t.rareBonus*100)}ポイント |`).join('\n')}
...
| 料理 | 基本売上 | 素材（各1個） | 解放 |
|---|---:|---|---|
${forestRecipes.map(r=>`| ${r.icon} ${r.name} | ${Math.round(r.price*GAME_CONFIG.saleMultiplier)} | ${r.requiredIngredients.map(id=>forestNames[id]).join('、')} | ${forestAreas.find(a=>a.secret===r.id)?.name}で切れ端3枚 |`).join('\n')}
