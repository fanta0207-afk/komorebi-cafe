...
- ミモザ雑貨店は${GAME_CONFIG.giftShopSize}種類を並べ、各ギフトはその品揃えにつき1個だけ購入できる。価格・レアリティの抽選重みは従来どおり。
...
- コイン発見時は${FOREST_COIN_BANDS.map(b=>`${b.min===b.max?b.min:b.min+'〜'+b.max}コイン：${b.weight}%`).join('、')}の重みで、各区間内は一様抽選。調達券1枚で発注1件の全パックを即受取。自然到着・消失済みの入荷には消費しない。
