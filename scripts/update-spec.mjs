...
- 調達券は最初の採取で1枚／2枚を各50%で確定。1回の探索で追加の券は出ない。切れ端だけ拾って帰る場合は出ない。
- かご初期10枠、持ち帰り3回で12、秘密レシピ2種類完成で14、4種類完成で16。普通食材・森の基本食材は1枠、物語限定食材・琥珀花蜜・月しずくベリーは2枠。コイン・券・切れ端は0枠。
- 満杯なら保留した採取結果を保存し、既存の持ち物を1個ずつ置いて入れるか、新しい食材を置く。採取の体力は返却しない。帰還は無料で体力0でも可能。受け取りで在庫・コイン・券・切れ端を一括加算し、二重受取できない。
- 森の食材で通常レシピの在庫条件を満たせば解放。仕入れ数・好感度は加算しない。
- 調達券は街の仕入れ先の各食材の調達中表示、またはカフェの在庫画面から使える。仕入れ先には所持枚数を表示し、発注直後に「調達券で即完了」を押せる。確認して1枚を消費し、選んだ入荷1件の全パックを即受取。スタッフの調達にも使える。自然到着・消失済みの入荷には消費しない。通常の入荷統計・レシピ解放処理を再利用。
...
- 探索中も営業・仕入れ・スタッフ作業は続行。完成皿・入荷待ち・空席を表示。新しい自動物語は帰還受取まで保留。下部ナビは帰還を経由。
