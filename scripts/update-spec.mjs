...
- コインを拾う確率は各採取で独立に${FOREST_CONFIG.coinChance*100}%。拾えたときの金額は少額中心の重み付き抽選。${FOREST_COIN_BANDS.map(b => `${b.min === b.max ? b.min : `${b.min}〜${b.max}`}コインは${b.weight}%`).join('、')}。各区間内は一様抽選。2000コインは大当たりで、4%はコインを拾えた場合の確率（採取全体では${FOREST_CONFIG.coinChance * FOREST_COIN_BANDS.find(b => b.min === 2000).weight}%）。601〜1999は出ない。複数回分を合算。既に保留・持ち帰り済みのコイン額は変更しない。
...
- 森の手帳は「食材」「秘密のレシピ」「限定料理」「手作り」の4ページ。長い説明文を省き、食材の発見・在庫・重さ、切れ端進捗、価格・調理秒数、必要素材と所持数、販売待ち・注文数、作成済みギフトを短く表示。必要素材数は選択した皿数に合わせる。食材の場所と深い森の解放条件は折りたたみ内に置く。指定の販売手順・包装代の説明文と販売上限の補足は表示しない。詳細は12〜13px、品名は15pxで表示し、素材ごとの行と場所ごとの段落に余白を置く。深い森の条件は項目別の2行に分ける。販売・取り下げ・手作りの操作は維持する。
