...
- 初回は最初の探索で森素材を保証し、3回までに小道を発見。以後は通常抽選。秘密のレシピの切れ端は対応層帯で発見でき、同じレシピは1探索で最大1枚、合計3枚で解放。泉の2レシピは未完成で進捗が少ない方を優先。
...
普通の自動注文抽選には入らない。手帳の「1〜3皿作る」で材料を消費し、完成料理をその場で所持する。解放と設置設備を確認し、通常の待ち注文の材料を守る。各料理の未提供合計は3皿まで。種類数の制限、販売開始、取り下げ、材料返却の操作はない。完成料理の所持があると、空席へ通常客と交互に来店。来店時に所持数を1減らし、完成した料理をすぐ提供できる。調理や材料消費を繰り返さない。注文を断ると完成品の所持へ戻る。所持・来店分は保存される。v22以前の販売待ちは完成料理の所持へ引継ぎ、既に調理中の注文は続行する。売上は通常の熟練度補正（最大20%）を適用。リクエスト25%上乗せは付けない。

| 料理 | 基本売上 | 素材（各1個） | 解放 |
|---|---:|---|---|
${forestRecipes.map(r=>`| ${r.icon} ${r.name} | ${Math.round(r.price*GAME_CONFIG.saleMultiplier)} | ${r.requiredIngredients.map(id=>forestNames[id]).join('、')} | ${r.hidden?'切れ端3枚':'最初の提供'} |`).join('\n')}
