...
import { GAME_CONFIG, relationshipNames } from "../game/config";
import { GameProvider, useGame } from "../game/GameContext";
import { availableEvent, bestSeller } from "../game/logic";
import type { DaySummary, RelationshipEvent } from "../types/game";
...
function GameContent() {
  ...
  const spawnOrder=useCallback(()=>{
    ...
    const recipeId=current.unlockedRecipes[Math.floor(Math.random()*current.unlockedRecipes.length)];
  ...
  useEffect(()=>{
    ...
    if(next){setEvent(next);setEventPage(0);}
  },[state,event]);
  ...
  const endDay=()=>{setSummary({...state.dailyStats,day:state.day,topRecipeId:bestSeller(state.dailyStats.recipeSales),news:[...state.dayNews]});setScreen("result");};
  ...
  return <main className="game-shell">
    ...
    <div className="screen-wrap">
      ...
      {screen==="character"&&characterId&&<CharacterDetail characterId={characterId} onBack={()=>setScreen("people")}/>} 
    ...
    {event&&<EventModal event={event} page={eventPage} onNext={()=>{if(eventPage<event.dialogue.length-1){setEventPage(eventPage+1);}else{dispatch({type:"COMPLETE_EVENT",eventId:event.id,characterId:event.characterId,toStage:event.toStage});setEvent(undefined);}}}/>} 
    {devOpen&&<DevMenu selected={devCharacter} onSelect={setDevCharacter} onClose={()=>setDevOpen(false)} onSpawn={spawnOrder} onRefresh={refreshGiftShop}/>} 
  </main>;
}

function EventModal({event,page,onNext}:{event:RelationshipEvent;page:number;onNext:()=>void}) {
  const character=getCharacter(event.characterId)!;
  return <div className="event-overlay"><div className="event-scene"><div className="event-banner"><span>RELATIONSHIP EVENT</span><h2>{event.title}</h2></div><Portrait character={character}/><div className="event-dialogue"><b>{character.name}</b><p>「{event.dialogue[page]}」</p><button onClick={onNext}>{page===event.dialogue.length-1?"この時間を心に刻む":"次へ"} →</button></div><div className="page-dots">{event.dialogue.map((_,i)=><i className={i===page?"active":""} key={i}/>)}</div></div></div>;
}

function DayResult({summary,onNext}:{summary:DaySummary;onNext:()=>void}) {
  const top=getRecipe(summary.topRecipeId||"");
  return <section className="result-screen fade-in"><span className="result-flower">✽</span><span className="tiny-label">CLOSE OF BUSINESS</span><h1>春 {summary.day}日<br/>営業結果</h1><div className="receipt"><div><span>本日の売上</span><strong>● {summary.sales.toLocaleString()}</strong></div><div><span>注文件数</span><strong>{summary.orders}件</strong></div><div><span>一番売れた料理</span><strong>{top?`${top.icon} ${top.name}`:"—"}</strong></div></div><div className="news-box"><b>新しい出来事</b>{summary.news.length?summary.news.map((item,i)=><p key={i}>♡ {item}</p>):<p>今日は穏やかな一日でした。</p>}</div><button className="primary-button" onClick={onNext}>翌日へ　→</button></section>;
}
