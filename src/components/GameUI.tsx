...
export function StatusBar({state,onMenu,menuLabel,missionControl}:{state:GameState;onMenu?:()=>void;menuLabel?:string;missionControl?:ReactNode}) {
  return <header className="topbar">
    <div className={`date-lockup ${missionControl?"with-mission":""}`}><span className="eyebrow">KOMOREBI CAFE</span>{missionControl}</div>
    <div className="status-actions"><div className="action-pill" title="お手伝い中のスタッフ"><span>♧</span><b>{state.staff.filter(person=>person.role!=="rest").length}</b><small>人</small></div>{onMenu&&<button type="button" className="dev-trigger" onClick={onMenu} aria-label={menuLabel||"設定"}>⚙</button>}<div className="coin-pill"><span>●</span> {state.currency.toLocaleString()}</div></div>
  </header>;
}

export const navItems = [
  {id:"cafe",icon:"▣",label:"店"},{id:"town",icon:"⌂",label:"街"},{id:"gifts",icon:"♧",label:"ギフト"},{id:"people",icon:"♡",label:"人物"},{id:"staff",icon:"♧",label:"スタッフ"},
] as const;
