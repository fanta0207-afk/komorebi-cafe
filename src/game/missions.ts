...
const introduction:Mission[]=[
  ...
  add("forest-gather","森で食材を1回採集する","気になる場所をタップ",10,"forest",s=>yes(s.forest.tutorialDone||s.forest.expedition?.harvested||s.forest.discovered.length)),
  add("forest-bring-home","森の食材を店へ持ち帰る","店へ帰ると自動で持ち帰り",15,"forest",s=>yes(s.forest.discovered.length)),
