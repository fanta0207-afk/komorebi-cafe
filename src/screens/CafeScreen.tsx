...
export function CafeScreen({state,onCollect,onEndDay}:{state:GameState;onCollect:(id:string)=>void;onEndDay:()=>void}) {
  ...
  return <section className="screen fade-in">
    ...
    {(visibleDecorations.length>0||visibleEquipment.length>0)&&<div className="cafe-growth-note"><span>✦ 店に残った思い出 {visibleDecorations.length}</span><span>設置設備 {visibleEquipment.length}</span></div>}
