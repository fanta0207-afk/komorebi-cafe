...
export function GiftShopScreen() {
  ...
  return <section className="screen gift-shop-screen fade-in"><ScreenTitle title="ミモザ雑貨店"><button className="refresh-button" disabled={!remaining} onClick={()=>refreshGiftShop(true)}>{remaining?"↻ 品揃えを更新":"本日の更新は終了"}</button></ScreenTitle>
    <div className="gift-shop-refresh-status"><span>手動更新 あと<b>{remaining}</b>/{GAME_CONFIG.giftShopDailyRefreshLimit}回<small>毎日0時に回復</small></span><span>次の自動更新 <b>{nextRefresh}</b><small>6時間ごと・日本時間</small></span></div>
