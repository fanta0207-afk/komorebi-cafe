...
export function MenuScreen({tab="equipment",onTabChange:setTab}:{tab?:"recipes"|"equipment";onTabChange:(tab:"recipes"|"equipment")=>void}) {
  ...
  return <section className="screen fade-in"><ScreenTitle kicker="CAFE GROWTH" title="設備と料理"/><p className="intro-copy">調理は店全体で1品ずつ、基本30秒です。解放した設備は店内に表示され、設置すると使えます。強化すると1段階ごとに15%短縮。増設した台では、別の台が提供待ちでも次の料理を作れます。</p>
    {tab==="equipment"&&<div className="investment-note"><b>売上を貯めて、お店に投資</b><p>コーヒー1杯：売上 {salePrice("coffee")} − 食材代 {ingredientCost("coffee")} = 利益 {salePrice("coffee")-ingredientCost("coffee")}コイン。基本設備の強化は400〜550コインから。仕入れ代も残しておきましょう。</p></div>}
