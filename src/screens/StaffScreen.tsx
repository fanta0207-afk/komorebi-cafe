...
export function StaffScreen() {
  const {state}=useGame();
  return <section className="screen fade-in"><ScreenTitle title="一緒に働く人"><small>雇用 {state.staff.length}/6人</small></ScreenTitle><div className="staff-list">{characters.map(character=><StaffCard key={character.id} characterId={character.id}/>)}</div></section>;
}
