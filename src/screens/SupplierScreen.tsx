...
import { SupplyTicketButton } from "../components/SupplyTicketButton";
...
export function SupplierScreen({supplierId,highlightIngredientId,onBack,onDate}:{supplierId:string;highlightIngredientId?:string;onBack:()=>void;onDate:(event:DateEvent)=>void}) {
  ...
  return <section className="screen fade-in">
    ...
    <p className="supplier-ticket-count">🎟 調達券 {state.forest.tickets}枚 · 仕入れ中に使うとすぐ届きます</p>
...
function SupplyItem({ item, highlighted, targetRef }: { item: Ingredient; highlighted: boolean; targetRef: RefObject<HTMLElement | null> }) {
  ...
  return <article ref={highlighted?targetRef:undefined} className={`supply-item ${item.limited ? "limited-item" : ""} ${highlighted ? "supply-item-highlight" : ""}`}>
    ...
    {pending.map(delivery => <div className="supply-pending" key={delivery.id}><p className="supply-delivery"><span><strong>{delivery.staffId?getCharacter(delivery.staffId)?.shortName||"スタッフ":"店長"}</strong>が {delivery.packs * delivery.servingsPerPack}食分を仕入れ中{delivery.automatic?"・自動":""}</span><b>あと {deliveryCountdown(delivery.arrivesAt, state.lastPlayedAt)}</b></p><SupplyTicketButton delivery={delivery}/></div>)}
  </article>;
}
