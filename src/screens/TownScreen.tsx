...
export function TownScreen({onOpen}:{onOpen:(supplierId:string)=>void}) {
  const {state}=useGame();
  return <section className="screen fade-in"><ScreenTitle kicker="OLD TOWN" title="春風通り"></ScreenTitle><p className="intro-copy">仕入れは、あなたの手で。何度でも訪問できます。新しい会話や仕入れ、贈物で少しずつ距離を縮めましょう。</p>
